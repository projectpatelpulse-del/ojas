...
class InvoiceService {
  ...
  static Future<pw.Document> _createDocument(Map<String, dynamic> order) async {
    ...
    // Fallback to currently logged in user if customer details are missing
    final loggedInUser = SessionService.instance.currentUser;
    if (loggedInUser != null) {
      if (customer['name'] == null || customer['name'].toString().trim().isEmpty) {
        customer['name'] = loggedInUser.name;
      }
      if (customer['email'] == null || customer['email'].toString().trim().isEmpty) {
        customer['email'] = loggedInUser.email;
      }
      if (customer['phone'] == null || customer['phone'].toString().trim().isEmpty) {
        customer['phone'] = loggedInUser.mobile;
      }
    }
