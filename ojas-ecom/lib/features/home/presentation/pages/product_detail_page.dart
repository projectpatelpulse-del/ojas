...
class _ProductDetailPageState extends State<ProductDetailPage> {
  ...
  void _initProductDetails(ProductModel p) {
    ...
    if (p.variations.isNotEmpty) {
      final targetVar = p.variations.firstWhere(
      ...
      _currentPrice = targetVar.price > 0 ? targetVar.price : p.price;
      _currentOldPrice = targetVar.oldPrice ?? p.oldPrice;
  ...
  void _updateSelectedVariation() {
    ...
    setState(() {
      ...
      _currentOldPrice = match.oldPrice ?? product.oldPrice;
      ...
      children: [
        ...
        Row(
          children: [
            ...
            Builder(
              builder: (context) {
                final double? effectiveOldPrice = _currentOldPrice ?? product.oldPrice;
                final bool showDiscount = effectiveOldPrice != null && effectiveOldPrice > _currentPrice;
                final int computedDiscount = showDiscount
                    ? (((effectiveOldPrice - _currentPrice) / effectiveOldPrice) * 100).round()
                    : 0;

                if (!showDiscount) return const SizedBox.shrink();
